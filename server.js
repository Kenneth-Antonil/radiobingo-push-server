...
db.ref('notifications').on('child_added', function(userSnap) {
  ...
  userSnap.ref.on('child_added', async function(notifSnap) {
    ...
    // SKIP 'pm' — the messages listener already sends a push for every new message.
    // index.html ALSO writes to notifications/ for PMs (type:'pm'), so without this
    // skip, every PM fires TWO pushes — one from here, one from the messages listener.
    if (type === 'pm') return;
